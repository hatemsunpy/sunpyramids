      ...
      <div v-if="catogries?.children.length" class="py-14 grid 2xl:grid-cols-4 xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6">
        <div v-for="item in catogries?.children" :key="item.id"
          ...
          <NuxtLink
            ...
            <NuxtImg class="w-full h-full group-hover:scale-110 transition-all" :src="item?.featured_image" loading="lazy" :alt="item?.title || 'Category image'" format="webp" width="400" height="224" />
