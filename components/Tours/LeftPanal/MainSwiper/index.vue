  ...
  <section v-if="tour" class="select-none">
    <swiper :spaceBetween="10" @slide-change="pause" :navigation="true" :thumbs="{ swiper: thumbsSwiper }"
      ...
      <swiper-slide
        ...
        <NuxtImg :src="img" class="w-full md:min-h-[36.25rem] md:max-h-[36.25rem] object-cover object-top" :loading="index === 0 ? 'eager' : 'lazy'" width="1000" height="580" format="webp" :alt="tour?.title ? `${tour.title} - gallery image ${index + 1}` : `Gallery image ${index + 1}`" />
    ...
    <swiper ref="tourSwiper" :navigation="true" @swiper="setThumbsSwiper" :breakpoints="{
      ...
      <swiper-slide v-for="(img, index) in tour?.gallery" :key="index" class="relative">
        <NuxtImg :src="img" class="w-full h-full object-cover" loading="lazy" width="64" height="64" format="webp" alt="" />
