...
// ─── Dev-focused statusline (compact one-liner) ─────────────────
function generateDevStatusline() {
  const git = getGitInfo();
  const modelName = getModelFromStdin() || getModelName();
  const ctxInfo = getContextFromStdin();

  let line = c.bold + modelName + c.reset;
  line += ' ' + c.dim + '│' + c.reset + ' ';
  line += c.brightBlue + path.basename(CWD) + c.reset;

  if (git.gitBranch) {
    line += ' ' + c.dim + '│' + c.reset + ' ';
    line += c.brightCyan + git.gitBranch + c.reset;
    if (git.modified > 0) line += ' ' + c.brightYellow + '~' + git.modified + c.reset;
    if (git.staged > 0) line += ' ' + c.brightGreen + '+' + git.staged + c.reset;
    if (git.untracked > 0) line += ' ' + c.dim + '?' + git.untracked + c.reset;
  }

  if (ctxInfo && ctxInfo.usedPct > 0) {
    line += ' ' + c.dim + '│' + c.reset + ' ';
    const ctxColor = ctxInfo.usedPct >= 90 ? c.brightRed : ctxInfo.usedPct >= 70 ? c.brightYellow : c.green;
    line += ctxColor + 'ctx ' + ctxInfo.usedPct + '%' + c.reset;
  }
...
if (process.argv.includes('--json')) {
  ...
  console.log(generateDevStatusline());
} else {
